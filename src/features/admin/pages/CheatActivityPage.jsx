...
const CheatActivityPage = () => {
  ...
  const fetchCheatActivity = async (assignmentId) => {
    try {
      ...
      setError('Failed to fetch cheat activity data');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Alert severity="info" sx={{ mb: 3 }}>
        Monitor exam integrity across all classes and detect suspicious activities during quiz submissions. Tab switches &gt; 1 or ESC exits are flagged for review.
      </Alert>
      ...
      <Paper sx={{ p: 3, mb: 3 }}>
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} md={6}>
            <FormControl fullWidth disabled={!selectedClass} sx={{ minWidth: 300 }}>
                        ...
                        <Box 
                          ...
                          onClick={() => {
                            setSelectedViolations({
                              candidateName: submission.candidateId?.name || 'N/A',
      ...
      {!selectedAssignment && !loading && (
        <Paper sx={{ p: 4, textAlign: 'center' }}>
          <Typography color="text.secondary">
            Please select a class and assignment to view cheat activity data.
          </Typography>
        </Paper>
      )}
...
export default CheatActivityPage;
